...
async fn main() {
    ...
    *state_with_wiki.wiki.write().unwrap_or_else(|e| e.into_inner()) = wiki_graph;
    ...
    let app = Router::new()
        ...
        .route("/jobs/:id/fragment", get(handlers::jobs::job_detail_fragment))
        ...
        .route("/workshop",          get(handlers::workshop::workshop_page))
        .route("/workshop/list",     get(handlers::workshop::workshop_list))
        .route("/jobs/manual",       post(handlers::jobs::submit_manual_job))
